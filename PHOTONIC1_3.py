...
def serialize_frame_v13(paths: list, bs: int, q_y: float, q_c: float, deadzone: float,
    ...
    # Rozdzielenie na bloki kwadratowe (planar) i krawędziowe (raw fallback)
    ...
    # 2. INTRA Edge (Raw fallback)
    if intra_edge:
        for p in intra_edge:
            raw_data = p['data']
            out.extend(struct.pack('>HHHHI', p['x'], p['y'], p['bh'], p['bw'], len(raw_data)))
            out.extend(raw_data)

    # 3. INTER Square (Planar)
    if inter_square:
    ...
    # 4. INTER Edge (Raw fallback)
    if inter_edge:
        for p in inter_edge:
            dx_c = max(-127, min(127, p['dx']))
            dy_c = max(-127, min(127, p['dy']))
            raw_data = p['data']
            out.extend(struct.pack('>HHbbHHI', p['x'], p['y'], dx_c, dy_c, p['bh'], p['bw'], len(raw_data)))
            out.extend(raw_data)
...
def deserialize_frame_v13(data: bytes, bs: int, q_y: float, q_c: float) -> list:
    """Zwraca listę dict jak v1.2 paths"""
    off = 0
