...
"""
╔═══════════════════════════════════════════════════════════════════════════════╗
║   PHOTONIC VIDEO CODEC v1.4 — HYBRYDA PLANARNO-TEMPORALNA + WEBP (FIXED)      ║
╠═══════════════════════════════════════════════════════════════════════════════╣
║  Poprawka stabilności:                                                        ║
║  - Zunifikowano obsługę marginesów w silniku planarnym (Zero-Padding)         ║
║  - Usunięto błąd KeyError: '_block' występujący przy niestandardowych wymiarach║
╚═══════════════════════════════════════════════════════════════════════════════╝
"""
...
_MAGIC      = b'PH05'     # Identyfikator formatu v1.4
_ZSTD_LEVEL = 19
...
# ═══════════════════════════════════════════════════════════════════════════════
# SERIALIZACJA I DESERIALIZACJA STRUMIENIA
# ═══════════════════════════════════════════════════════════════════════════════

def serialize_frame_v14(paths: list, bs: int, q_y: float, q_c: float, deadzone: float,
                         h: int, w: int) -> bytes:
    out = bytearray()
    ...
    if len(paths) == 1 and paths[0]['type'] == 0:
        out.append(0)  # I-frame (WebP Payload)
...
def deserialize_frame_v14(data: bytes, bs: int, q_y: float, q_c: float) -> list:
    off = 0
...
class PhotonicCodec:
    ...
    def encode_frame(self, frame_rgb: np.ndarray) -> list:
        ...
        # ─── I-FRAME: COMPRESSION WITH WEBP ───
        if self.prev_wave is None:
            webp_bytes = iio.imwrite("<bytes>", frame_rgb[..., :3], extension=".webp", quality=self.webp_quality)
            recon_rgb = iio.imread(webp_bytes, extension=".webp")
            wave3 = rgb_to_ycbcr(recon_rgb)
            paths.append({'type': 0, 'data': webp_bytes})
        ...
        # 1. Main Grid Loop (Bloki wewnętrzne)
        for row in range(h_blocks):
            for col in range(w_blocks):
                if static_mask[row, col] if row < h_blocks and col < w_blocks else False:
                    continue
        ...
        # 2. Margins Loop (FIXED: Zero-Padding do bs x bs w celu eliminacji KeyError)
        if h % bs != 0 or w % bs != 0:
            for ey in range(0, h, bs):
                for ex in range(0, w, bs):
                    ...
                    brush_padded = np.zeros((bs, bs, 3), dtype=np.float32)
                    brush_padded[:bh, :bw] = wave3[ey:ey+bh, ex:ex+bw]
                    
                    prev_brush_padded = np.zeros((bs, bs, 3), dtype=np.float32)
                    prev_brush_padded[:bh, :bw] = self.prev_wave[ey:ey+bh, ex:ex+bw]
                    
                    dy, dx, ncc = optical_correlate(prev_brush_padded, brush_padded)
                    if ncc >= self.energy_thr:
                        delta = extract_interference_delta(prev_brush_padded, brush_padded, dy, dx)
                        delta_mask = np.max(np.abs(delta), axis=-1) > self.delta_thr
                        if np.any(delta_mask):
                            sparse_delta = delta * delta_mask[..., np.newaxis]
                            paths.append({
                                'type': 1, 'x': ex, 'y': ey, 'dx': dx, 'dy': dy,
                                'bh': bh, 'bw': bw, '_block': sparse_delta
                            })
                    else:
                        paths.append({
                            'type': 2, 'x': ex, 'y': ey, 'dx': 0, 'dy': 0,
                            'bh': bh, 'bw': bw, '_block': brush_padded
                        })
    ...
    def decode_frame(self, paths: list, h: int, w: int) -> np.ndarray:
        if self.prev_wave is None:
            webp_bytes = paths[0]['data']
            recon_rgb = iio.imread(webp_bytes, extension=".webp")
            self.prev_wave = rgb_to_ycbcr(recon_rgb)
            return recon_rgb
        ...
        for path in paths:
            ...
            if path['type'] == 1:
                ...
                if '_block' in path:
                    new_wave[y:y+bh, x:x+bw] = path['_block'][:bh, :bw]

        self.prev_wave = new_wave
        return ycbcr_to_rgb(new_wave)

# ═══════════════════════════════════════════════════════════════════════════════
# TEMPORAL CONTEXT (ZSTD DICTIONARY FOR P-FRAMES)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# MENEDŻER I/O PLIKÓW PHOTONIC
# ═══════════════════════════════════════════════════════════════════════════════

@contextmanager
def photonic_writer(path, w, h, fps, bs, q_y, q_c, deadzone, webp_quality=90):
    f = open(path, 'wb')
    f.write(_MAGIC)
    f.write(struct.pack('>HHfHfff', w, h, fps, bs, q_y, q_c, deadzone))
    dict_builder = TemporalDictBuilder()
    codec_enc = PhotonicCodec(brush_size=bs, q_y=q_y, q_c=q_c, deadzone=deadzone, webp_quality=webp_quality)
    ...
    try:
        class W:
            ...
            def write_frame(self, frame_rgb: np.ndarray):
                ...
                raw = serialize_frame_v14(paths, bs, q_y, q_c, deadzone,
...
# ═══════════════════════════════════════════════════════════════════════════════
# CLI OBSŁUGA STRUMIENIOWANIA
# ═══════════════════════════════════════════════════════════════════════════════

def encode_video(inp, out, args):
    if not iio: return print("Brak imageio")
    print("[Photonic v1.4] Enkodowanie (I-Frame: WebP | P-Frame: Planar+Zstd Temporal)...", flush=True)
    ...
    with photonic_writer(out, w, h, args.fps, args.brush_size,
                         args.q_y, args.q_c, args.deadzone, args.webp_quality) as writer:
    ...
    print(f"\n[Photonic] Zapisano pomyślnie {fi} klatek.", flush=True)


def decode_video(inp, out, args):
    if not iio: return print("Brak imageio")
    print("[Photonic v1.4] Dekodowanie strumienia do formatu wideo...", flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--webp-quality',    type=int,   default=90, help="Jakość kompresji stratnej WebP dla I-Frame (1-100)")
