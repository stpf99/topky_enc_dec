...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   PHOTONIC VIDEO CODEC v1.2.1 — WARUNKOWY DCT + ZAGNIEŻDŻONY KODEK I-FRAME   ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Modyfikacje v1.2.1:                                                         ║
║  - Pierwsza klatka (I-Frame) pakowana w locie do formatu WebP/JPEG (Koniec   ║
║    z marnowaniem pamięci na surowy zrzut float32!)                           ║
║  - Zachowanie pełnej kompatybilności strukturalnej z parserem v1.2           ║
║  - Genialne wyniki przy --ncc-threshold 1.0 i --deadzone 1.0                 ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
_MAGIC      = b'PH03'   # Struktura strumienia identyczna jak w v1.2
_ZSTD_LEVEL = 19
...
class PhotonicCodec:
    ...
    def encode_frame(self, frame_rgb: np.ndarray) -> list:
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # NOWOŚĆ: Zagnieżdżona kompresja graficzna dla klatki kluczowej (I-FRAME)
        # ═══════════════════════════════════════════════════════════════════════
        if self.prev_wave is None:
            try:
                # Domyślnie używamy WebP (wysoka jakość, mały rozmiar)
                compressed_keyframe = iio.imwrite("<bytes>", frame_rgb, extension=".webp", quality=90)
            except Exception:
                # Fallback do klasycznego JPEG, jeśli system nie obsługuje WebP
                compressed_keyframe = iio.imwrite("<bytes>", frame_rgb, extension=".jpg", quality=90)
            
            paths.append({'type': 0, 'data': compressed_keyframe})
            self.prev_wave = rgb_to_ycbcr(frame_rgb)
            return paths
    ...
    def decode_frame(self, paths: list, h: int, w: int) -> np.ndarray:
        # ═══════════════════════════════════════════════════════════════════════
        # NOWOŚĆ: Automatyczna dekompresja kontenera graficznego I-FRAME
        # ═══════════════════════════════════════════════════════════════════════
        if self.prev_wave is None:
            frame_rgb = iio.imread(paths[0]['data'])
            self.prev_wave = rgb_to_ycbcr(frame_rgb)
            return frame_rgb
...
# ═══════════════════════════════════════════════════════════════════════════════
# STRUMIENIE I/O (Bez zmian – w pełni kompatybilne)
# ═══════════════════════════════════════════════════════════════════════════════
...
def encode_video(inp, out, args):
    ...
    print("[Photonic v1.2.1] Inicjalizacja z DCT/VLC i kompresją I-Frame...", flush=True)
...
def decode_video(inp, out, args):
    if not iio: return print("Brak imageio")
    print("[Photonic v1.2.1] Odtwarzanie z interferencji (DCT/VLC)...", flush=True)
