...
def process_row_pframe(args):
    """
    Przetwarza jeden rząd makrobloków klatki P.
    Wywoływana równolegle przez ProcessPoolExecutor.
    Zwraca: (y, lista wyników dla każdego bloku w rzędzie)
    """
    (y, w_pad, h_pad,
     prev_Y, curr_Y,
     prev_U, curr_U,
     ...
     use_subpixel, mad_row) = args  # mad_row: tablica MAD per kolumna dla tego wiersza
    ...
    while x < w_pad:
        ...
        # --- Estymacja ruchu UV (wektory = połowa wektorów Y, bo 4:2:0) ---
        cy, cx = y // 2, x // 2
        bs_c = bs // 2

        # Wektor UV = wektor Y podzielony przez 2 (w pikselach)
        uv_dy = best_dy_qp // 8  # quarter-pixel → half-pixel dla UV
        uv_dx = best_dx_qp // 8

        csy_c = cy + uv_dy
        csx_c = cx + uv_dx
        csy_c = max(0, min(csy_c, prev_U.shape[0] - bs_c))
        csx_c = max(0, min(csx_c, prev_U.shape[1] - bs_c))
...
class TopTopowCodecV2:
    ...
    def _encode_pframe(self, curr_Y, curr_U, curr_V, h_pad, w_pad):
        ...
        for row_idx, y in enumerate(range(0, h_pad, self.block_size)):
            ...
            tasks.append((
                y, w_pad, h_pad,
                self.prev_Y.copy(), curr_Y,
                self.prev_U.copy(), curr_U,
                self.prev_V.copy(), curr_V,
    ...
    def _decode_pframe(self, data, h, w):
        ...
        for block in data['blocks']:
            ...
            # Guard: blok nie moze wychodzic poza granice klatki
            if y + bs > h or x + bs > w:
                continue
            ...
            csx_c = max(0, min(cx + uv_dx, self.prev_U.shape[1] - bs_c))
            new_U[cy:cy+bs_c, cx:cx+bs_c] = np.clip(
                self.prev_U[csy_c:csy_c+bs_c, csx_c:csx_c+bs_c] +
                apply_idct2(block['q_diff_u'].astype(np.float32) * self.Q_C),
                -128, 127)
            new_V[cy:cy+bs_c, cx:cx+bs_c] = np.clip(
                self.prev_V[csy_c:csy_c+bs_c, csx_c:csx_c+bs_c] +
